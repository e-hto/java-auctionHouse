...
public class AuctionHouseTest {
    ...
    @Test
    public void testAddLotNoSeller(){
        logger.info(makeBanner("testAddLotNoSeller"));
        runStory(2);
        assertError(house.addLot("SellerW",2,"Painting", new Money("200.00")));
    }
    ...
    @Test
    public void testNoteInterestSoldLot() {
        logger.info(makeBanner("testNoteInterestSoldLot"));
        runStory(8);
        assertError(house.noteInterest("BuyerA", 1));
    }

    @Test
    public void testNoteInterestDuplicate() {
        logger.info(makeBanner("testNoteInterestDuplicte"));
        runStory(4);
        assertError(house.noteInterest("BuyerB", 1));;

    }

    @Test
    public void testOpenAuctionNoInterest(){
        logger.info(makeBanner("testOpenAuctionNoInterest"));
        runStory(3);
        assertError(house.openAuction("Auctioneer1", "@Auctioneer1", 1));
    }

    @Test
    public void testOpenAuctionNotUnsold() {
        logger.info(makeBanner("testOpenAuctionNotUnsold"));
        runStory(4);
        house.addLot("SellerZ", 7, "Chair", new Money("110.00"));
        house.openAuction("Auctioneer2","@Auctioneer2",7);
        assertError(house.openAuction("Auctioneer2","@Auctioneer2",7));
        house.noteInterest("BuyerA",7);
        house.openAuction("Auctioneer2","@Auctioneer2",7);
        house.makeBid("BuyerA",7,new Money("130"));
        house.closeAuction("Auctioneer2", 7);
        assertError(house.openAuction("Auctioneer2","@Auctioneer2",7));
    }
    ...
    @Test
    public void testMakeBidNoInterest() {
        logger.info(makeBanner("testMakeBidNoInterest"));
        runStory(7);
        assertError(house.makeBid("BuyerC",1, new Money("200")));
    }

    @Test
    public void testMakeBidNotInAuction(){
        logger.info(makeBanner("testMakeBidNotInAuction"));
        runStory(4);
        assertError(house.makeBid("BuyerA", 1, new Money("70")));
    }

    @Test
    public void testWrongAuctioneerCloseAuction(){
        logger.info(makeBanner("testWrongAuctioneerCloseAuction"));
        runStory(7);
    ...
    @Test
    public void testCloseAuctionWithSalePending() {
        logger.info(makeBanner("testCloseAuctionWithSalePending"));
        runStory(7);
        bankingService.setBadAccount("BB A/C");
        assertEquals(house.closeAuction("Auctioneer1",  1).kind,Status.Kind.SALE_PENDING_PAYMENT);
    }

    @Test
    public void testNoSale() {
        logger.info(makeBanner("testNoSale"));
        runStory(7);
        bankingService.setBadAccount("BB A/C");
        assertEquals(house.closeAuction("Auctioneer1",  1).kind,Status.Kind.SALE_PENDING_PAYMENT);
    }
